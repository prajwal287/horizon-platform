"""dlt pipeline: Kaggle 1-3m-linkedin-jobs-and-skills-2024 → GCS (Parquet), full replace. Step 1 of dlt → GCS → BigQuery."""
import logging
import os
...
def jobs_resource() -> Iterator[dict]:
    for batch in stream_kaggle_linkedin_jobs_skills_2024():
        for row in batch:
            yield row
...
def run() -> dlt.Pipeline:
    ...
    logger.info("Kaggle linkedin_jobs_skills_2024 pipeline load_info: %s", load_info)
    return pipeline
