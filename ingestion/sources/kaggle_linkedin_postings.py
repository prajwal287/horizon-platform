"""Kaggle arshkon/linkedin-job-postings (2023-2024): load, filter, yield batches."""
import logging
...
SOURCE_ID = "kaggle_linkedin_postings"
SOURCE_NAME = "Kaggle LinkedIn Job Postings 2023-2024"
# Conservative default if no date column
DEFAULT_POSTED_DATE = date(2023, 6, 1)
BATCH_SIZE = 10_000


def _infer_column_map(df: pd.DataFrame) -> dict[str, str]:
    """Infer CSV column -> canonical from common LinkedIn-style names."""
    col_map: dict[str, str] = {}
    for c in df.columns:
        s = str(c).lower().strip()
        if "title" in s or "job_title" in s:
            col_map[c] = "job_title"
...
def _skills_to_list(val: Any) -> List[str] | None:
    ...
    if isinstance(val, str):
        if val.startswith("[") or ";" in val or "|" in val:
            return [x.strip() for x in val.replace(";", "|").replace("[", "").replace("]", "").split("|") if x and x.strip()]
...
def stream_kaggle_linkedin_postings(
    ...
    if batch:
        yield batch
    logger.info("Kaggle linkedin_postings: yielded %d rows after filters", count)
