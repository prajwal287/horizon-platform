...
4. **ingestion/sources/*.py** (e.g. `kaggle_data_engineer_2023.py`)  
   - Download or open the source (e.g. Kaggle CSV).  
   - Read in chunks; for each row map columns to canonical names and build a `RawJobRow`.  
   - Optionally fill `skills` from title/description (taxonomy) if `EXTRACT_SKILLS_TAXONOMY=1`.  
