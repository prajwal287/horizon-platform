...
Each pipeline does the same pattern:
...
3. **Create a dlt pipeline** with:
   - a **resource**: a Python generator that yields rows (dicts).
...
```
  ...
  dlt.pipeline(destination="filesystem", dataset_name=...)
    │
    ▼
  pipeline.run(_jobs_resource(), table_name="jobs", write_disposition="replace", ...)
    │
    ├──────────────────────────────────────┐
    │  _jobs_resource()                    │
    │    │                                  │
...
### Step 4: Where does the data come from? (Sources)
...
### Step 5: Where do the rows go? (GCS)
...
- **Pipelines** set **DESTINATION__FILESYSTEM__BUCKET_URL** to something like `gs://BUCKET/raw/huggingface_data_jobs` or `gs://BUCKET/raw/kaggle_data_engineer_2023`.
...
## 4. How Is the Connection to GCP Made?
...
## 5. Where Are Files and Secrets Stored Locally?

### 5.1 GCP credentials (for GCS/BigQuery etc.)
...
### 5.2 Project ID and GCS bucket name
...
## 6. How Is Docker Set Up to Run Locally?
...
- **Copy**: `ingestion/` and `run_ingestion.py` into `/app`.
...
### 6.2 docker-compose.yml (run stage)
...
## 7. Terraform’s Role (GCP Resources)
...
## 8. Quick Reference: Commands and What They Do
...
4. **Pipeline**: sets dlt destination to `gs://BUCKET/raw/kaggle_data_engineer_2023`, creates a pipeline with a resource that yields rows.
...
For diagrams that render in GitHub or VS Code (with a Mermaid extension), see **CODEBASE_FLOWCHARTS.md** in this folder. It has:
